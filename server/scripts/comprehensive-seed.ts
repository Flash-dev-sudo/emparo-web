...
async function seedComprehensiveMenu() {
  try {
    console.log("Clearing existing menu items...");
    await db.delete(menuItems);

    console.log("Inserting comprehensive menu based on till system structure...");
    const menuData = [
      // Starters Category
      {
        name: "Chips",
    ...
    for (let i = 0; i < menuData.length; i += batchSize) {
      ...
      console.log(`Inserted batch: ${totalInserted}/${menuData.length} items`);
    }
